...
public class ChessGameBoard extends JPanel{
    ...
    private void nextTurn(){ //board
        currentPlayer = ( currentPlayer == 1 ) ? 2 : 1;
        ( (ChessPanel)board.getParent() ).getGameLog().addToLog(
                "It is now Player " + currentPlayer + "'s turn." );
    }
    public int getCurrentPlayer(){ board
        return currentPlayer;
    }
    private boolean selectedPieceIsValid(){ //board
    ...
    public boolean isKingInCheck( boolean checkCurrent ){ //board
        if ( checkCurrent ){
            if ( currentPlayer == 1 ){
    ...
    private void checkGameConditions(){ //board
